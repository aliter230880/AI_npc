import { getStoredAuthToken, refreshAuthSession } from "./auth";
...
export type DiagnosticsSnapshot = {
  auth: { enabled: boolean; jwt: boolean; apiKey: boolean };
  llm: { provider: string; enabled: boolean; model: string };
  stt: { provider: string; enabled: boolean; model: string };
  tts: { provider: string; enabled: boolean; model: string; voice: string };
  realtime: { ws: boolean };
};
...
export async function getCharacterMemories(
  ...
  if (shouldUseRemoteApi()) {
    try {
      return await request<CharacterMemory[]>(
        `/characters/${characterId}/memories?limit=${limit}&includeExpired=${includeExpired}`
      );
    } catch {
      // fall through to local fallback
    }
...
export async function addCharacterMemory(
  ...
  return request<CharacterMemory>(`/characters/${characterId}/memories`, {
    method: "POST",
...
export async function updateCharacterMemory(
  characterId: string,
  memoryId: string,
  payload: Partial<Pick<CharacterMemory, "content" | "priority" | "pinned" | "expiresAt">>
): Promise<CharacterMemory> {
  return request<CharacterMemory>(`/characters/${characterId}/memories/${memoryId}`, {
    method: "PATCH",
    body: JSON.stringify(payload),
  });
}

export async function deleteCharacterMemory(characterId: string, memoryId: string): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>(`/characters/${characterId}/memories/${memoryId}`, {
    method: "DELETE",
  });
}

export async function listKnowledgeDocuments(characterId: string): Promise<KnowledgeDocument[]> {
  return request<KnowledgeDocument[]>(`/characters/${characterId}/knowledge`);
}
...
export async function uploadKnowledgeDocument(
  ...
  const response = await fetch(`${apiBase}/characters/${characterId}/knowledge`, {
    method: "POST",
    headers: {
      ...(apiKey ? { "x-api-key": apiKey } : {}),
      ...(token ? { Authorization: `Bearer ${token}` } : {}),
...
export async function deleteKnowledgeDocument(characterId: string, documentId: string): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>(`/characters/${characterId}/knowledge/${documentId}`, {
    method: "DELETE",
  });
}
...
export async function runTtsDiagnostics(): Promise<{ ok: boolean; codec: string; chunks: number }> {
  return request<{ ok: boolean; codec: string; chunks: number }>("/diagnostics/tts", {
    method: "POST",
    body: JSON.stringify({ text: "Quick diagnostics check" }),
  });
}

export async function checkApiHealth(): Promise<boolean> {
  const healthUrl = getResolvedApiBase().replace(/\/v1\/?$/, "") + "/health";
  const response = await fetch(healthUrl);
  return response.ok;
}

export async function checkWsReady(): Promise<boolean> {
  const endpoint = resolveWsUrl();
  if (!endpoint) {
    return false;
  }
